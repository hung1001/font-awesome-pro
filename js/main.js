...
removeDuplicate.forEach(value => {
  ...
  if (ver) {
    ...
    $('#archive').append(`<option data-link='https://github.com/${REPO}/archive/v${ver}.zip'>v${ver}</option>`);
  }
});
...
$('.ver').text(ver);
$('#a').val(`<link href="https://cdn.jsdelivr.net/gh/${REPO}@${sha}/css/all.css" rel="stylesheet" type="text/css" />`);
$('#c').val(`<link href="https://cdn.staticaly.com/gh/${REPO}/${sha}/css/all.css" rel="stylesheet" type="text/css" />`);
$('#e').val(`<link href="https://rawcdn.githack.com/${REPO}/${sha}/css/all.css" rel="stylesheet" type="text/css" />`);
$('head').append($('#a').val());

$('body').on('select2:select', '#list-versions', function() {
  let newSHA = $(this).val();
  const replaceSHA = (ele, sha = newSHA) => $(ele).val().replace(/[a-f0-9]{7}\//i, `${sha}/`);
